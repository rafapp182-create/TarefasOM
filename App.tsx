  ...
  if (loading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-zinc-950">
        ...
        <p className="text-black dark:text-zinc-400 font-black uppercase tracking-widest text-[10px] animate-pulse">Sincronizando OmPro...</p>
      </div>
    );
  }
    ...
    switch(currentView) {
      case 'dashboard': 
        return profile.role === 'executor' ? (
          <Dashboard profile={profile} grupos={grupos} activeGroupId={activeGroupId} setActiveGroupId={setActiveGroupId} />
        ) : (
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-zinc-950 overflow-hidden relative transition-colors duration-300">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between px-4 z-30">
        ...
        <div className="flex items-center gap-2 bg-gray-50 dark:bg-zinc-800 px-3 py-1.5 rounded-full border border-gray-100 dark:border-zinc-700">
          <span className="text-[10px] font-black text-black dark:text-white uppercase tracking-tighter truncate max-w-[120px]">
            {profile.name}
          </span>
        </div>
      </div>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black/60 z-40 md:hidden backdrop-blur-[2px]" onClick={() => setIsSidebarOpen(false)} />
      )}
